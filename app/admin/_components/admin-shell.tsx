    ...
    <main className="min-h-screen bg-slate-100">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-5 md:flex">
            ...
            <Link href="/user" className="text-sm font-medium text-slate-500 hover:text-slate-800">
              User view
            </Link>
          </nav>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur md:hidden">
        <div className="grid grid-cols-4 gap-2">
          ...
          <Link href="/user" className="rounded-lg px-2 py-2 text-center text-xs font-semibold text-slate-600">
            User
          </Link>
        </div>
      </nav>
