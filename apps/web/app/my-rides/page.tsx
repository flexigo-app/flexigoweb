...
import { formatSummaryDate } from "@/lib/booking-summary";
...
const getBusinessDateValue = (offsetDays = 0) => {
...
const getDaysRemaining = (dateStr: string): number | null => {
  const trip = new Date(`${dateStr}T00:00:00Z`);
  const today = new Date(`${getBusinessDateValue()}T00:00:00Z`);
...
const formatCardDate = (dateStr: string, timeStr: string, meridiem: "AM" | "PM") => {
  const prefix = dateStr === getBusinessDateValue()
    ? "Today"
    : dateStr === getBusinessDateValue(-1)
      ? "Yesterday"
      : formatSummaryDate(dateStr);

  return `${prefix}, ${timeStr} ${meridiem} ET`;
};
            ...
            {filtered.map((ride) => {
              ...
              return (
                <div key={ride.id} className="overflow-hidden rounded-[20px] border border-[#E2EDF6] bg-white shadow-sm">
                  ...
                  {ride.status === "cancelled" && daysLeft !== null && daysLeft < 0 && (
                    <div className="mx-4 mb-4 mt-3 flex items-start justify-between gap-3 rounded-2xl border border-[#FCA5A5] bg-[#FEF2F2] px-4 py-3 text-sm text-[#991B1B]">
                      <div>
                        <p className="font-bold">This unpaid booking has expired</p>
                        <p className="mt-0.5 text-xs font-normal opacity-80">The scheduled trip date passed before payment was received.</p>
                      </div>
                      <Link href="/user" className="shrink-0 rounded-full border border-[#FCA5A5] bg-white px-3 py-1.5 text-xs font-bold text-[#991B1B] hover:bg-[#FEE2E2]">
                        Re-book
                      </Link>
                  ...
                  {/* Payment is available only before the scheduled service date. */}
                  {ride.status === "confirmed" && (
                    <div className="border-t border-[#EDF3F9] px-4 pb-4 pt-3">
                      {daysLeft === 0 ? (
                        <div className="flex items-start justify-between gap-3 rounded-2xl border border-[#FCD34D] bg-[#FFFBEB] px-4 py-3 text-sm text-[#92400E]">
                          <div>
                            <p className="font-bold">Your trip is today</p>
                            <p className="mt-0.5 text-xs font-normal opacity-80">Contact support to confirm whether a driver is still available.</p>
                          </div>
                          <a href="mailto:support@flexigo.com" className="shrink-0 rounded-full border border-[#FCD34D] bg-white px-3 py-1.5 text-xs font-bold text-[#92400E] hover:bg-[#FEF3C7]">
                            Support
                          </a>
                        ...
                        <div className="flex items-start justify-between gap-3 rounded-2xl border border-[#FCA5A5] bg-[#FEF2F2] px-4 py-3 text-sm text-[#991B1B]">
                          <div>
                            <p className="font-bold">This booking has expired</p>
                            <p className="mt-0.5 text-xs font-normal opacity-80">The trip date passed before payment was completed.</p>
                          </div>
                          <Link href="/user" className="shrink-0 rounded-full border border-[#FCA5A5] bg-white px-3 py-1.5 text-xs font-bold text-[#991B1B] hover:bg-[#FEE2E2]">
                            Re-book
                          </Link>
                        ...
                        <button type="button" className="inline-flex h-11 w-full items-center justify-center rounded-full bg-gradient-to-r from-[#0B83E9] to-[#38B6FF] text-sm font-bold text-white shadow-md transition hover:brightness-105">
