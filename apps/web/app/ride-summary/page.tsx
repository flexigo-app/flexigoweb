...
const subscribeToBookingSummary = () => () => {};

const getBookingSummarySnapshot = () =>
  typeof window === "undefined"
    ? null
    : window.sessionStorage.getItem(bookingSummaryStorageKey);

export default function RideSummaryPage() {
  const router = useRouter();
  const storedSummary = useSyncExternalStore(
    subscribeToBookingSummary,
    getBookingSummarySnapshot,
    () => null
  );
  ...
  const totalFare = bookingSummary?.estimatedFareCents === undefined
    ? null
    : bookingSummary.estimatedFareCents / 100;
                  ...
                  <div className="mt-5 rounded-[22px] border border-[#E2EEF9] bg-white px-5 py-4 shadow-sm">
                    <div className="flex items-end justify-between gap-4">
                      ...
                      <div className="text-right">
                        <p className="text-3xl font-extrabold text-[#102A43]">{totalFare === null ? "Calculated at request" : `$${totalFare.toFixed(2)}`}</p>
